...
case class RabbitConfig(config: Config) {
  ...
  private val poolSize = rabbit.get[Int]("poolSize")
  private val xName    = exchange.get[String]("name")
  private val xType    = exchange.get[String]("type")

  private val routingKey = rabbit.get[String]("routingKey")

  def toAmqpProtocol: AmqpProtocol = {
    AmqpProtocol.default.copy(
      connection = Connection(host = host, port = port, vhost = vhost, user = user, password = password, poolSize = poolSize, routingKey = routingKey),
